...
const handleStorageChange = (event: StorageEvent) => {
  if (event.key === 'minha_colecao' || event.key === 'historico_removidos') {
    carregarDados();
  }
};
...
.banner { background: rgba(255,255,255,0.03); padding: 12px; text-align: center; font-weight: 700; text-transform: uppercase; letter-spacing: 2px; margin-bottom: 25px; border-left: 5px solid transparent; }
...
.keyboard-card { background: var(--cinza-card); padding: 25px; border-radius: 12px; border: 1px solid rgba(255,255,255,0.05); position: relative; margin-bottom: 15px; transition: 0.3s; }
...
.btn-clear-outline { background: transparent; border: 1px solid var(--primaria); color: var(--primaria); font-size: 0.7rem; padding: 5px 15px; border-radius: 4px; font-weight: 800; cursor: pointer; transition: 0.3s; }
